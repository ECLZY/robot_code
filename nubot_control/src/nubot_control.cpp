...
public:
	...
	nh_ = boost::make_shared<ros::NodeHandle>(robot_name);
        ...
        action_cmd_pub_ = nh_->advertise<nubot_common::ActionCmd>("nubotcontrol/actioncmd",0.2);
...
//        shoot_client_ = nh_->serviceClient<nubot_common::Shoot>(service1);
        worldmodelinfo_sub_ = nh_->subscribe("worldmodel/worldmodelinfo", 0.4, &NuBotControl::update_world_model_info,this);
        ballisholding_sub_ = nh_->subscribe("ballisholding/BallIsHolding",0.4,&NuBotControl::update_ballisholding,this);
...
//            }
    ...
    bool  Checkline(DPoint pos, DPoint tar)
    {
        double target_radian_  = (tar-pos).angle().radian_;
        for(int i=0;i<OUR_TEAM;i++)
        {
            if(abs((world_model_info_.Opponents_[i] - pos).angle().radian_-target_radian_)<0.1)
                return false;
        }
        return true;
    }
...
//            parking_target.x_ = -900;//守门员站在离球门最近的地方
    ...
    //上层:持球进攻策略//=300,=world_model_info_.field_info_.oppGoal_[GOAL_MIDDLE]
    bool attack(double shoot_radius=300)
    {
        DPoint goal(1100,0);
        //射球
        if (robot_pos_.distance(goal)<shoot_radius)
        {
            action_cmd_.move_action = TurnForShoot;
            action_cmd_.rotate_acton= TurnForShoot;
            action_cmd_.rotate_mode = 0;
            move2ori((goal-robot_pos_).angle().radian_,robot_ori_.radian_);
            {
                ...
                if(robot_ori_.radian_>low_radian_ && robot_ori_.radian_<up_radian_)
                {
                    ...
                    action_cmd_.strength = (goal-robot_pos_).length()/100;
        ...
        {
            //传球
            world_model_info_.caculatePassPosition();
    ...
    //斥力模型
    bool repulsion(DPoint goal, double distance_thres=30.0)
    {
        // double K1=80, K2=6000, K3=1000000, acc_radius=300;
        double K1=200, K2=10000, K3=1000000, acc_radius=300;
        DPoint acceleration, vel, pos_relative, force;
        vel = world_model_info_.RobotInfo_[world_model_info_.AgentID_-1].getVelocity();
        //目标引力,K1级数
        acceleration.x_ = K1*(goal.x_-robot_pos_.x_)/goal.distance(robot_pos_);
        acceleration.y_ = K1*(goal.y_-robot_pos_.y_)/goal.distance(robot_pos_);
        //障碍物斥力，K2/300级数
        acc_radius+=world_model_info_.RobotInfo_[world_model_info_.AgentID_-1].getVelocity().length();
        ROS_FATAL("robotpos:%lf, %lf",robot_pos_.x_,robot_pos_.y_);
        for(int i=0;i<world_model_info_.Opponents_.size();i++)
        {
            double dist = robot_pos_.distance(world_model_info_.Opponents_[i]);
            if(dist<acc_radius)
            {
                // repulsion_1.0 K2=6000
                // acceleration-=K2/dist/dist*(world_model_info_.Opponents_[i]-robot_pos_);
                // repulsion 2.0 
                if(goal.distance(robot_pos_)<300)
                    acceleration -= K2*(1/dist-1/acc_radius)*pow(goal.distance(robot_pos_),2)/pow(dist,3)*(world_model_info_.Opponents_[i]-robot_pos_);
                else
                    acceleration -= K2*(1/dist-1/acc_radius)*pow(300,2)/pow(dist,3)*(world_model_info_.Opponents_[i]-robot_pos_);
                // 仅考虑相对方向
                // pos_relative = pos_relative.rotate(90*DEG2RAD);
                // if(force.dot(pos_relative)>=0)
                //     acceleration-=force.dot(pos_relative)/pow(pos_relative.length(),2)*pos_relative;
                // else
                //     acceleration+=force.dot(pos_relative)/pow(pos_relative.length(),2)*pos_relative;
            }
        }
        //持球情况下--边界斥力，K3/100/100,现已保证600-700以tanx函数
        if (world_model_info_.RobotInfo_[world_model_info_.AgentID_-1].getDribbleState())
        if (fabs(robot_pos_.y_)>600)
        {
            // 纯斥力方法 K3=1e6
            // acceleration.y_-=K3*robot_pos_.y_/fabs(robot_pos_.y_)*fabs(tan(SINGLEPI_CONSTANT/300*(fabs(robot_pos_.y_)-600)));
            // 考虑速度 K3=300
            acceleration.y_ = acceleration.y_/10;
            if(vel.y_*robot_pos_.y_>0)
            {
                acceleration.y_ -= K3*robot_pos_.y_/fabs(robot_pos_.y_)*fabs(tan(SINGLEPI_CONSTANT/1600*(fabs(vel.y_))));
            }
        }
        

        
        if (robot_pos_.distance(goal)<distance_thres)
            return true;
        else if(move2ori(vel.angle().radian_,robot_ori_.radian_))
            s_goahead(acceleration, robot_pos_);
        return false;
    }

    //短距直线行进_保持方向前进
    bool s_goahead(DPoint acc, DPoint pos, double PID=1, double distance_thres=20.0, double angle_thres = 8.0*DEG2RAD)
    {
        double length =acc.length();
        
        if(PID>1)PID=1;
        //位移
        action_cmd_.target.x = pos.x_ + PID*acc.x_;
        action_cmd_.target.y = pos.y_ + PID*acc.y_;
        action_cmd_.maxvel = length;
        //角度
        DPoint vel;
        vel = world_model_info_.RobotInfo_[world_model_info_.AgentID_-1].getVelocity();
        action_cmd_.target_ori =vel.angle().radian_;
        action_cmd_.maxw = fabs(vel.angle().radian_-robot_ori_.radian_)*2;
        
        if(length > distance_thres||fabs(vel.angle().radian_-robot_ori_.radian_) > angle_thres)
            return false;
        else
            return true;
    }
    ...
    //长距直线行进_保持方向前进
    bool l_goahead(DPoint target, DPoint pos, double distance_thres=20.0, double angle_thres = 8.0*DEG2RAD)
    {
        DPoint t2r = target - pos;
        double length =pos.distance(target);
        
        //位移
        action_cmd_.target.x = pos.x_ + 10*t2r.x_/length;
        action_cmd_.target.y = pos.y_ + 10*t2r.y_/length;
        action_cmd_.maxvel = length;
        //角度
        action_cmd_.target_ori =t2r.angle().radian_;
        action_cmd_.maxw = fabs(t2r.angle().radian_-robot_ori_.radian_)*2;
        
        if(length > distance_thres||fabs(t2r.angle().radian_-robot_ori_.radian_) > angle_thres)
            return false;
        else
            return true;
    }

    //根据己方位置与障碍物相对位置得到是否堵塞
    bool jaming()
    {
        double acc_radius=150;
        int count=0;
        for(int i=0;i<world_model_info_.Opponents_.size();i--)
        {
            if(robot_pos_.distance(world_model_info_.Opponents_[i])<=acc_radius && world_model_info_.Opponents_[i].x_>=robot_pos_.x_)
            count ++;
        }
    ...
    void normalGame()
    {
        ...
        {
            DPoint complement(50.0,50.0);
            DPoint b2r = ball_pos_ - robot_pos_;
            DPoint tmp(200.0,400.0);
